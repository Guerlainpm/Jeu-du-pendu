...
document.getElementById("coopButton").addEventListener("click" , function(event){
  ...
  document.getElementById("form").style.WebkitFlexDirection = "column";
  document.getElementById("form").style.WebkitAlignItems = "center";
  document.getElementById("coopButton").style.display = "none";
  document.getElementById("soloButton").style.display = "none";
  document.getElementById("title").innerHTML = "Inserer le mot a chercher";
...
document.getElementById("soloButton").addEventListener("click" , function(event){
  document.getElementById("coopButton").style.display = "none";
  document.getElementById("soloButton").style.display = "none";
  document.getElementById("title").innerHTML = "A toi de trouver !";
  ...
  document.getElementById("reload").style.display = "block";
    ...
    wordToFind = tab[Math.floor(Math.random() * tab.length)].toUpperCase();
      hiddenWord = "_".repeat(wordToFind.length);
      event.preventDefault();
      document.getElementById("searchWord").style.display = "flex";
      document.getElementById("searchWord").innerHTML = hiddenWord;
...
function checkAndReplace(letter) {
  ...
  if (!splitHidden.includes("_") || erreursRestante == 0) {
    ...
    document.getElementById("title").innerHTML = "Bravo. Tu a trouver le mot " + '"' + wordToFind + '"' + " ." + "Bien jouer";
